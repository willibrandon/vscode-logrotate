...
export async function loadGrammar(scopeName: string): Promise<IGrammar> {
  ...
  const paths: Readonly<Record<string, string>> = {
    "source.logrotate": "syntaxes/logrotate.tmLanguage.json",
    "source.logrotate.state": "syntaxes/logrotate-state.tmLanguage.json",
    "source.logrotate.markdown": "syntaxes/logrotate-markdown.tmLanguage.json",
  };
  const registry = new Registry({
    ...
    async loadGrammar(requested): Promise<IRawGrammar | null> {
      ...
      if (path === undefined) {
        return requested === "source.shell"
          ? ({ scopeName: "source.shell", patterns: [], repository: {} } as unknown as IRawGrammar)
          : null;
      }
